    ...
    if (equal(a, b)) {
        ...
        var calculateMatch = function (primaryEntries, secondaryEntries, object_a, object_b, isSubMatch) {
            ...
            for (var ipde in primaryDateEntries) {
                if (utils.dateEqualMatch(object_a[primaryDateEntries[ipde].path], object_b[primaryDateEntries[ipde].path])) {
            ...
            for (var irre in primaryResourceReferenceEntries) {
                ...
                if (equal(inputRROA, inputRROB)) {
                    ...
                    primaryCount++;
                } else if (inputRROA.reference.trim().toLowerCase() === inputRROB.reference.trim().toLowerCase()) {
                    // if they have the same reference, equals 100% match
                    matchPercent = matchPercent + secondaryResourceReferenceEntries[irre].percent;
                    if (!isSubMatch) {
                        matchedPaths.push(secondaryResourceReferenceEntries[irre].path);
                    }
                } else if (inputRROA.display.trim().toLowerCase() === inputRROB.display.trim().toLowerCase()) {
                    // in case both have the same display, but different reference, the result will be half of the percent
                    matchPercent = matchPercent + (secondaryResourceReferenceEntries[irre].percent * 0.5);
                    if (!isSubMatch) {
                        matchedPaths.push(secondaryResourceReferenceEntries[irre].path);
                    }
                }
            ...
            if (matchPercent > 0) {
                ...
                for (var isde in secondaryDateEntries) {
                    if (utils.dateEqualMatch(object_a[secondaryDateEntries[isde].path], object_b[secondaryDateEntries[isde].path])) {
