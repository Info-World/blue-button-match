...
exports.matchSections = function matchSections(new_record, master, compare_field) {
    ...
    _.each(scr_ids, function (srcIdIndex) {
        ...
        var isPartial = _.findIndex(partials, { match: "partial", percent: _.max(partials, function (part) { return part.percent; })}); // get the highest level
        ...
        if (isDuplicate >= 0) {
            ...
            finalRes.src_id = results[isPartial].src_id;
            finalRes.match = 'partial';
            finalRes.percent = results[isPartial].percent;
            finalRes.master_id = results[isPartial].master_id;
            finalRes.subject_id = results[isPartial].subject_id;
        } else {
