...
var removeSourceDuplicates = require("./match-sections.js").removeSourceDuplicates;
...
var match = function match(new_record, master, log) {
    ...
    _.each(resourcesList, function (section) {
        ...
        var removeDuplicateFromSource = removeSourceDuplicates(new_section, name, log);
        new_section = removeDuplicateFromSource.new_records;
        ...
        //add duplicate from within the bundle
        if (removeDuplicateFromSource.cleaned_records && removeDuplicateFromSource.cleaned_records.length) {
            _.each(removeDuplicateFromSource.cleaned_records, function (record) {
                result.result.push(record.result);
            });
        }
    });
